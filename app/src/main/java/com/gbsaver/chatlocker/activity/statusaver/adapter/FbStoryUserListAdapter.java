...
public class FbStoryUserListAdapter extends RecyclerView.Adapter<FbStoryUserListAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder viewHolder, int position) {
        ...
        Glide.with(context).load(profilePic)
                .thumbnail(0.2f).into(viewHolder.storyPc);
        viewHolder.RLStoryLayout.setOnClickListener(view -> userListInterface.fbUserListClick(position, trayModelArrayList.get(position)));
    }
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        @BindView(R.id.story_pc)
        CircleImageView storyPc;
        @BindView(R.id.real_name)
        TextView realName;
        @BindView(R.id.user_name)
        TextView userName;
        @BindView(R.id.RLStoryLayout)
